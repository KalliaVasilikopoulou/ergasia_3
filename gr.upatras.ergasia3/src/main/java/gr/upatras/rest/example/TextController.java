...
//	
			...
			public ResponseEntity<Text> createText(@ApiParam(value = "The Text to be created", required = true)
			@RequestBody Text t) {
				log.info( "Will add a new text" );
				
				
				SimpleMqttClient smc = new SimpleMqttClient("grupatras/lab/engine/ergasia3");
				smc.runClient(t.getTxt());
				
				
				return new ResponseEntity<Text>( t, HttpStatus.OK);
			}
}
