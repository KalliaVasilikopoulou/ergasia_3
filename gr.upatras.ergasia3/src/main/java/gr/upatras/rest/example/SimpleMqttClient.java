...
//public static final String TOPIC = "grupatras/lab/engine/ergasia3";
public String TOPIC="";

//class constructor

public SimpleMqttClient(String topic) {
	super();
	TOPIC = topic;
}
...
*
*/
public void runClient(String val) {
	// setup MQTT Client
	...
	try {
	int subQoS = 0;
	myClient.subscribe(myTopic, subQoS);		//as a client, subscribe to the topic 
	if (!publisher) {
	while (true) {
	Thread.sleep(1000);						//after subscribing to the topic, wait for news/updates
	}
	}
	} catch (Exception e) {
	e.printStackTrace();
	...
	String pubMsg = "{\"value\":" + val + "}";		//message from publisher to the topic
	int pubQoS = 0;
